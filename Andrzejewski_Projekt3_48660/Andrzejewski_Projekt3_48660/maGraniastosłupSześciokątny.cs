...
{
    class maGraniastosłupSześciokątny : maWielościany
    {
        float maOś_Duża, maOś_Mała;
        float maKątMiędzyWierzchołkami, maKątPołożenia;
        Point[] maWielokątPodłogi;

        public maGraniastosłupSześciokątny(int maPromień, int maWysokość, int maStopieńWielokątaPodstawy, int maXŚrodekSufitu, int maYŚrodekSufitu, int maXŚrodekPodłogi, int maYŚrodekPodłogi, Color maKolorLinii, DashStyle maStylLinii, int maGrubośćLinii) : base(maPromień, maKolorLinii, maStylLinii, maGrubośćLinii)
        {
            this.maRodzajBryły = maTypBryły.ma_GraniastosłupSześciokątny;
            ...
            float maBokA = (float)(maOś_Duża / Math.Sqrt(2));
            float maPolePodstawy = (maBokA * 2);
            float maPoleBoku = (maBokA * maWysokość);
            this.maObjętośćBryły = (maPolePodstawy * maWysokość);
            this.maPowierzchniaBryły = (maPolePodstawy * 2 + maPoleBoku * 4);
        }
