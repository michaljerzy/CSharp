...
{
    class maOstrosłup : maBryłaAbstrakcyjna
    {
        ...
        public maOstrosłup(int maPromien, Color maKolorLinii, DashStyle maStylLinii, int maGruboscLinii, int maWysokosc, int maStopienWielokataPodstawy, Point maSrodekPodlogi) : base(maPromien, maKolorLinii, maStylLinii, maGruboscLinii)
        {
            maRodzajBryły = maTypBryły.maOstrosłup;
        ...
        void maObjętoścIPolePowierzchni()
        {
            switch (maStopieńWielokątaPodstawy)
            {
                case 3:
                    ...
                    maPowierzchniaBryły = (float)((3 * maPromień * maPromień * Math.Sqrt(3)) / 4 + 3 * (3 * maPromień) / Math.Sqrt(3)
                        * Math.Sqrt(Math.Pow((1 / 2) * maPromień, 2) + maWysokośćBryły * maWysokośćBryły) * 1 / 2);
                    break;
                case 4:
                    maObjętośćBryły = (1 / 3) * (float)Math.Pow((maOśDuża / Math.Sqrt(2)), 2) * maWysokośćBryły;
                    maPowierzchniaBryły = (float)(Math.Pow((maOśDuża / Math.Sqrt(2)), 2) + 2 * (maOśDuża / Math.Sqrt(2)
                        * Math.Sqrt(maPromień * maPromień + maWysokośćBryły * maWysokośćBryły)));
                    break;
                default:
                    maObjętośćBryły = (1 / 3) * (float)(maStopieńWielokątaPodstawy * ((3 * maPromień * maPromień * Math.Sqrt(3)) / 4) * maWysokośćBryły);
                    maPowierzchniaBryły = (float)(maStopieńWielokątaPodstawy * ((3 * maPromień * maPromień * Math.Sqrt(3)) / 4) + maStopieńWielokątaPodstawy
                        * (1 / 2) * (3 * maPromień / Math.Sqrt(3)) * (Math.Sqrt(Math.Pow((1 / 2) * maPromień, 2) + maWysokośćBryły * maWysokośćBryły)));
                    break;
            }
        }
