...
{
    class maBrylant : maBryłaAbstrakcyjna
    {
        ...
        public maBrylant(int maPromien, Color maKolorLinii, DashStyle maStylLinii, int maGruboscLinii, int maWysokosc, int maStopienWielokataPodstawy, Point maSrodekPodlogi) : base(maPromien, maKolorLinii, maStylLinii, maGruboscLinii)
        {
            maRodzajBryły = maTypBryły.maBrylant;
            ...
            maDolnySufit.X = maXŚrodkaPodłogi;
            maDolnySufit.Y = maYŚrodkaPodłogi + maWysokośćBryły;
        ...
        public override void maWykreśl()
        {
            ...
            for (int i = 0; i < maWielokątPodłogi.Length - 1; i++)
            {
                ...
                maRysownica.DrawLine(maPen, maDolnySufit, maWielokątPodłogi[i]);
            }
        ...
        public override void maZmieńWysokośćBryły(int maWys)
        {
            ...
            maDolnySufit.Y = maYŚrodkaPodłogi + maWys;
        ...
        void maObjętoścIPolePowierzchni()
        {
            ...
            {
                case 3:
                    maObjętośćBryły = 2 * ((1 / 3) * (float)((3 * maPromień * maPromień * Math.Sqrt(3)) / 4) * maWysokośćBryły);
                    maPowierzchniaBryły = 2 * ((float)((3 * maPromień * maPromień * Math.Sqrt(3)) / 4 + 2 * (3 * maPromień) / Math.Sqrt(3) * maWysokośćBryły));
                    break;
                case 4:
                    maObjętośćBryły = 2 * ((1 / 3) * (float)Math.Pow((maOśDuża / Math.Sqrt(2)), 2) * maWysokośćBryły);
                    maPowierzchniaBryły = 2 * ((float)(Math.Pow((maOśDuża / Math.Sqrt(2)), 2) + 2 * (maOśDuża / Math.Sqrt(2) * Math.Sqrt(maWysokośćBryły * maWysokośćBryły + maWysokośćBryły * maWysokośćBryły))));
                    break;
                default:
                    maObjętośćBryły = 2 * ((1 / 3) * (float)(maStopieńWielokątaPodstawy * ((3 * maPromień * maPromień * Math.Sqrt(3)) / 4) * maWysokośćBryły));
                    maPowierzchniaBryły = 2 * ((float)(maStopieńWielokątaPodstawy * ((3 * maPromień * maPromień * Math.Sqrt(3)) / 4) + maStopieńWielokątaPodstawy * (1 / 2) * (3 * maPromień / Math.Sqrt(3)) * (Math.Sqrt(maPromień * maPromień + maWysokośćBryły * maWysokośćBryły))));
                    break;
            }
        }
